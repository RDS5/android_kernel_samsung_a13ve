 ...
 */
/*----------------------------------------------------------------------------*/
int32_t nanNdpEndReqHandlerImpl(
	struct ADAPTER *prAdapter,
	struct _NAN_CMD_DATA_END *prCmd)
{
	struct MSG_NAN_NDP_DATA_END *prMsg =
		(struct MSG_NAN_NDP_DATA_END *) NULL;

	do {
		if (!prAdapter || !prCmd)
			break;

		prMsg = (struct MSG_NAN_NDP_DATA_END *)
			cnmMemAlloc(prAdapter, RAM_TYPE_MSG,
				sizeof(struct MSG_NAN_NDP_DATA_END));
		if (!prMsg) {
			DBGLOG(NAN, WARN,
				"Buffer not enough, can not end ndp.\n");
			break;
		}
		kalMemZero(prMsg,
			sizeof(struct MSG_NAN_NDP_DATA_END));

		prMsg->rMsgHdr.eMsgId =
			MID_CNM_NAN_NDP_DATA_END;

		kalMemCopy(&prMsg->rNanCmdDataEnd,
			prCmd,
	...
	for (i = 0; i < instanceIdNum; i++) {
		if (nanGetFeatureIsSigma(prGlueInfo->prAdapter))
			...
			rNanCmdDataEnd.ndp_instance_id = nla_get_u32(
				tb[MTK_WLAN_VENDOR_ATTR_NDP_INSTANCE_ID_ARRAY] +
				i * sizeof(uint32_t));
		rStatus = nanNdpEndReqHandlerImpl(
			prGlueInfo->prAdapter,
			&rNanCmdDataEnd);
	}
	return rStatus;
